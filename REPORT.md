...
答え:
.gitignore に以下の行を追記しました。
data/
これにより、点群データを配置した data ディレクトリが git のコミット対象外となるようにしました。
...
コマンド:
```bash
poetry run python main.py > exec.log 2>&1
```
...
コマンド:
```bash
# data/ を .dockerignore に追加済み
echo "data/" >> .dockerignore

# Docker イメージをビルド
docker build -t pointcloud-app .

# Docker コンテナを実行（data をマウント）
docker run --rm -v $(pwd)/data:/code/data pointcloud-app python main.py
```
