...
def icp_trimmed(
    ...
    for i in range(max_iterations):
        ...
        # Trim: select only top trim_ratio points with smallest errors
        sorted_idx = np.argsort(errors)
        k = int(len(errors) * trim_ratio)
        keep_idx = sorted_idx[:k]
        trimmed_src = src[keep_idx]
        trimmed_matched = matched[keep_idx]

        R, t = best_fit_transform(trimmed_src, trimmed_matched)
        src = (R @ src.T).T + t
        R_total = R @ R_total
        t_total = R @ t_total + t

        rmse = compute_rmse(trimmed_src, trimmed_matched)
        rmse_history.append(rmse)

        print(f"[Trimmed ICP] Iter {i}: RMSE = {rmse:.6f}")
        if i > 0 and abs(rmse_history[-2] - rmse) < tolerance:
            print(
                f"[Trimmed ICP] Converged at iter {i} with delta RMSE {abs(rmse_history[-2] - rmse):.6e}"
            )
            break
