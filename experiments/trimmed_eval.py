...
from src.pointcloud_registration.icp_algorithm_trimmed import icp_trimmed
...
def main() -> None:
    ...
    print("[LOG] Trimmed ICP 実行中...")
    aligned_sample, rmse_history, R_icp, t_icp = icp_trimmed(
        source_sample_aligned, target_sample_aligned, trim_ratio=0.9, max_iterations=200
    )
    print(f"[Trimmed Sample] Final RMSE: {rmse_history[-1]:.6f}")
    print(f"[Trimmed Sample] Iterations: {len(rmse_history)}")
    ...
    print("[LOG] 結果を保存中...")
    plot_rmse_history(rmse_history, save_path="results/rmse_plot_trimmed.png")
    save_pointcloud_plot(
        aligned_full, target_eval, save_path="results/pointclouds_trimmed.png"
    )
    print("[LOG] 完了：結果を results/ に保存しました。")
